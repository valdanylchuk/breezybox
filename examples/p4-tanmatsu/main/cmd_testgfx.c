/*
 * cmd_testgfx.c - Built-in VGA 320x200 graphics demo (`testgfx`).
 *
 * On-device smoke test for the graphics path (no ELF needed): enters VGA13h,
 * bounces colored rectangles and rotates the palette, then returns to text.
 * Ported from the S3 breezydemo; uses the rgb_display_* / rgb_gfx_* API now
 * provided by breezy_tanmatsu_lcd.
 */
...
/* Smooth 6-segment rainbow across palette entries 1-255 (0 stays black). */
...
int cmd_testgfx(int argc, char **argv)
{
    ...
    int x = 10, y = 10, vx = 1, vy = 1, bw = 40, bh = 30;
    uint8_t color = 1;
    int frame = 0;
    int64_t start = esp_timer_get_time();

    while (frame < max_frames) {
        if (use_vsync) rgb_display_wait_vsync();
        ...
        /* Palette rotation: shift entries 1..255; framebuffer untouched. */
        uint16_t tmp = pal[1];
    ...
    double fps = (double)frame / (elapsed / 1000000.0);
